...
class TransformerDataset(Dataset):
    ...
    def get_batch_design(self, batch_size, include_labels=False):
        X, y = self.raw.get_batch_design(batch_size, include_labels=True)
        X, y = self.transformer.perform(X, y)
        if include_labels:
            return X, y
        return X
    ...
    def iterator(self, mode=None, batch_size=None, num_batches=None,
        ...
        raw_iterator = self.raw.iterator(mode, batch_size, num_batches, topo, True, rng)
        
        final_iterator = TransformerIterator(raw_iterator, self, targets=targets)
...
class TransformerIterator(object):
    ...
    def __init__(self, raw_iterator, transformer_dataset, targets):
    ...
    def next(self):
        ...
        """
        raw_batch = self.raw_iterator.next()
        if self.raw_iterator_targets:
            rval = (self.transformer_dataset.transformer.perform(raw_batch[0]), raw_batch[1])
        else:
            rval = self.transformer_dataset.transformer.perform(raw_batch[0], raw_batch)
        return rval
        """
